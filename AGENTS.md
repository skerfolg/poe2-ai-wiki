...
**「크다」고 말하기 전에 `scan_scalers`로 분해한다 (#102).** 「X당 Y」 문구를 그냥
...
**「이 둘을 같이 쓰면 하나가 죽나」는 `scan_antagonists`로 (#131).** 위 그래프들은 전부
**생산·소비**를 본다(A가 X를 만들고 B가 X를 쓴다). **적대는 축이 다르다** — A가 X를
만드는데 B가 **X 자체를 금지**해 담체 한 칸이 통째로 죽는다. 실측 2026-08-27:
`Xoph's Pyre`(Fire→Extra Chaos)와 불의 화신(Deal no Non-Fire)이 정면 적대해 두 구성의
최종 DPS가 **1,556,109로 동일**했다 — 젬 한 칸이 완전 무효인데 아무 신호가 없어
**사용자가 인게임에서 발견했다**. `carrier_ids=`로 좁히면 「지금 이 조합에서 무엇이
죽나」다 — **키스톤을 찍기 전에 물어라**. 담체는 14종뿐이지만 파급이 크다(Brutality
I~III가 각 233종, 불의 화신이 188종을 죽인다). 조립 출고(`assemble_pob`)에는
`antagonists`로 **자동으로 실린다**. ⛔ **거부가 아니라 신고다** — 적대라도 대가를 알고
쓰는 선택일 수 있어 판정은 호출자 몫(AD-3).

**「이 조건부 노드를 살리려면 무엇이 필요한가」는 `scan_enablers`로 (#125).**
`scan_state_edges`(#92)의 54축은 전부 **가하거나 생성하는** 상태다(동결·점화·충전).
조건부 노드가 요구하는 것은 **자신이 처한 상태**(낮은 생명력·포위·이동 중)로 **축이
다르다** — 실측 2026-08-27: 셋 다 그 54축에 없다. ⭑ **주체가 축에 있다** — 「Low Life」는
자신과 적 양쪽에 쓰이고 한 레코드가 둘을 함께 갖는다(`support.execute-iii`가 대표,
혼재 4종). 레코드 단위로 읽으면 낮은 생명력 빌드가 **자기한테 맞는 젬을 후보에서 잃는다**.
⛔ **`satisfy_by`를 먼저 볼 것** — `resource`·`situation`만 「무엇을 사야 하나」에 답이
있고, `equipment`는 슬롯을 채우면 되고 `behaviour`(이동·정지·채널링)는 **살 것이 없다**.
그 축의 성립기 0건을 수집 갭으로 읽으면 **없는 갭을 쫓는다**. 성립기는 세 부류다 —
`redefine`(`You count as on Low Life while at 35% of maximum **Mana**`) ·
`force`(회복 상한을 묶는다) · `relax`(포위 문턱 −2~4).

**「이 표기 딜이 팩에서도 서나」는 `audit_config_upkeep`으로 (#145).** `compute_pob`은
config를 **그대로 받아** 계산할 뿐이라, 조건을 전부 켜 둔 남의 빌드를 태우면 그 수치가
「몹팩마다 여섯 종을 다 걸고 반경 안에 서 있을 때」의 값인지 **아무 반환값도 말하지
않았다**. 실측 2026-09-04(레퍼런스 힘스태킹 녹아내린 폭발 젬링): 세팅 없이 팩에 들어가면
표기의 **52.6%**였고, 켜 둔 13종 중 **6종은 Δ0**(켤 이유조차 없다)·**원소 노출 3종은
끄는 쪽이 높았다**(**역저항 축을 쓰는 빌드라면** 원소 노출·저항 감소는 역효과다 — 담체와
실측은 BACKLOG #145; 그 빌드의 젬 이름은 여기 적지 않는다).
정적 신고는 `compute_pob` 반환의 **`config_upkeep`에 자동으로 붙고**(자원 관문·대상
한계·횟수·반경·쿨다운, 근거는 KB 문구 **원문**), `measure=True`면 조건별 실제 기여와
`all_off_pct`까지 잰다. ⚠ **마크는 `Limit 1 Marked targets`라 팩에서 한 마리에만
걸린다** — PoB는 대상 하나를 재므로 상시 걸림으로 계산된다. ⛔ **거부가 아니라
신고다**(AD-3) — 보스 하나를 오래 잡는 구도면 전부 거는 게 맞다.

**「인게임 검증 필요」로 적기 전에 인사이트 층부터 — `search_insights`·`get_insight` (#137).**
인사이트는 **라벨이 가장 높은 층**이다(IN_GAME · scope=durable) — 사용자가 인게임에서
확인해 준 것, 이미 빌드를 폐기시킨 것이 여기 쌓인다. 메커니즘·젬 이름으로
`search_insights`를 한 번 치고 설계한다. 실측 2026-09-03: 「CoMD가 트리거한 스킬은 다른
메타 젬의 에너지를 만들지 않는다」가 `insight.meta-gem-trigger-chain-limits`에 인게임
확인으로 적혀 있었는데 지도에 경로가 없어 **같은 오류로 두 번째 빌드가 폐기됐다**.
⚠ 문서만으로는 안 지켜진다(철칙 5) — 트리거 도구 반환에 인사이트를 자동 부착하는
강제 지점은 아직 없다(#137 미완).
...
**희귀는 두 형식이고 쓰임이 다르다 — 선언은 명세용, 문구는 계산용 (#148).**
`optimize_rare`가 내는 `spec_text`(`Prefix: <모드키>`)는 **PoB `Craft()`에 먹이는 명세**다.
빌드 스펙의 `items[].text`에 그대로 넣으면 **접사가 하나도 안 들어간다** — 소켓·룬은
먹으므로 잘 조립된 것처럼 보이고 수치만 낮다(실측 2026-09-09: 접두를 바꿔 두 번 돌렸는데
DPS가 **소수점까지 동일**했다). 계산에는 **렌더된 `text`**를 쓰고, 명세밖에 없으면
`pok.pob.roundtrip.build_items`로 렌더한 뒤 넣는다. ⚠ `check_item_legality`의
`legal: True`는 **「계산된다」가 아니다** — 선언형은 줄별 전부 LEGAL이면서 계산이 빈다.
그래서 `compute_pob`은 `unbuilt_declarations`를, 검사기는 `not_computable`을 **양쪽에서**
낸다(한쪽만 신고하면 통과한 쪽만 보고 넘어간다).

**Item은 이름이 아니라 속성으로 묻는다 — `category`·`sub_type` (#146·#147).**
아이템에 닿는 축이 이름 하나뿐이라 세션이 이름 토큰으로 훑었고, 그러면 갭이 **0건이
아니라 그럴듯한 부분집합**으로 나와서 안 보인다. 실측 2026-09-09: `query="Boots"`로
16종을 보고 **「신발에는 회피/ES 듀얼 베이스가 없다」**고 판정해 보고까지 했는데,
실제로는 `category="boots"`가 **217건**이고 이름군이 Boots(회피)·Sandals(ES)·
Greaves(방어도)로 갈려 있었다 — 그 조합은 **27종** 있다. Item의 `tags`는 **비어
있으니** 부위 필터로 쓰지 말 것. 표기 분포는 `describe_type("Item", field="sub_type")`.
✅ 효과 문구로도 이제 찾힌다(유니크의 `explicits`가 색인 밖이었다 — 세션이 피스메이커를
**이름만 들고 30분** 찾았다).
