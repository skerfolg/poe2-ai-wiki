...
3. **엔진=결정적** — `src/poe2_wiki/engine/`엔 빌드 솔버·생성 판단을 넣지 않는다. 판단은 `skills/`+에이전트.
4. **인덱스는 코드가 자동 재생성** — `var/index.sqlite`를 손대거나 커밋하지 말 것. `src/poe2_wiki/index/`의 self-healing이 처리(없음/KB수정/버전업 감지).
