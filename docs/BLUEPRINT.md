...
> **문서 상태**: v0.7 (2026-07-31) — 빌드 생성의 중심을 "트리 최적화"에서 **"설계 루프(제약 충족 + 가설 관리)"**로 재정의 (§10.0, D25~D30, 사용자 협의 2026-07-31 — 사용자 검토 대기)
> **목적**: 코드 작성 전, 확정한 방향·아키텍처·계획을 **빠짐없이 기록**하고 남은 결정을 명시한다. 세부 구현이 아니라 **방향·러프 플랜** 수준. **이 문서가 프로젝트의 단일 진실 소스(source of truth)다.**
> **이력**: v0.1(도구형) → v0.2(엔진+MCP) → v0.3(프로토타입 회수) → v0.4(프로토타입=참고용, 스냅샷 폐기) → v0.5(생성 전략·트리 최적화 확정) → v0.6(구조·KB모델·ingest·로드맵 확정, 패키지명 `pok`) → **v0.7(설계 루프 상위 계층, D25~D30)**
> **상세 문서**: [PROJECT_STRUCTURE.md](PROJECT_STRUCTURE.md)(물리 구조) · [KB_DATA_MODEL.md](KB_DATA_MODEL.md)(KD-1~4) · [KB_INGEST.md](KB_INGEST.md)(KI-1~7) · [ROADMAP.md](ROADMAP.md)(P0~P6) · [BUILD_DESIGN.md](BUILD_DESIGN.md)(설계 문서 규격, D26)
...
| D25 | 설계 루프 | 빌드 생성의 상위 계층 = **7단계 설계 루프(제약 충족 + 가설 관리)**. §10.2·§10.3은 그 5~6단계의 도구 (사용자 확립 2026-07-31) |
| D26 | 설계 문서 | **design.md = 1급 산출물** — 규격 [BUILD_DESIGN.md](BUILD_DESIGN.md), 원형=사용자 v6 문서 |
| D27 | 제약 충족 | 포인트 예산·색상 장부·점유·자원 = **이산 제약 원장** + 엔진의 **결정적 제약 검사기**(예정) |
| D28 | 목적 형태 | **목표 상태(임계값·캡) 우선** — 평평한 가중 합산은 경계·배타 구조를 표현 못 함 |
| D29 | 가설 관리 | 설계 판단에 **확정/잠정/미검증 3구분**(검증 라벨의 설계 버전). 미검증 가설 목록 = **P5 입력 형식** |
| D30 | 앵커 빌드 | 검증된 외부 앵커 = `artifacts/anchors/`(parse_pob 수집, 계보 manifest) — RC5 "근거 있는 재조합"의 앵커 공급 |
...
### 10.0 설계 루프 (상위 계층) — D25~D30 (사용자 협의 2026-07-31, 초안)

빌드 생성의 중심을 재정의한다: 중심은 "트리 최적화"가 아니라 **설계 루프 = 제약 충족 + 가설 관리**다. §10.1~10.3의 기존 결정(파이프라인·트리 최적화·RC1~5)은 전부 유지되며, 그 **상위에** 이 계층이 놓인다. 근거: 사용자의 실제 설계 실천(불씨 일제 사격 인퍼널리스트 v6, 2026-07-31 세션)에서 검증된 작업 방식의 일반화 — 규격은 `docs/BUILD_DESIGN.md`에 옮겨져 있다.

**① 7단계 설계 루프** (사용자 확립 2026-07-31):

1. 빌드 컨셉 정하기
2. 컨셉 기재의 현실적 수행 가능성 검토
3. 최적 직업군·어센던시 파악
4. 최소 구동 조건 확인
5. 필수 어센던시·패시브·아이템 확정
6. 스킬 리스트업·보조젬 매칭
7. 지속 반복으로 문제 수정·새 가능성 발견

⑦이 ①~⑥으로 되먹임되는 **루프**다. 기존 §10.2 생성 파이프라인은 이 루프의 실행 뷰이고, §10.3 트리 최적화는 **5~6단계 안에서 동작하는 도구**다.

**② 제약 충족 계층 (D27)** — 설계의 본체는 연속량 최적화가 아니라 **이산 제약 충족**이다. v6에서 실증된 제약 유형:

- **포인트 예산 배타** — 전직 8포인트에 4묶음(10포인트어치)을 다 넣을 수 없어 분기가 강제됨
- **색상 장부** — 결정화된 면역 `빨강 보조 수 > 전체 ÷ 2` 같은 전 빌드 스코프 집계 조건
- **점유 상한·하한** — 점유 효율 65% 초과 시 잔여 생명력 35%를 넘어 로우라이프가 풀림
- **자원 소진** — 성유 중복 불가 등 되돌릴 수 없는 1회성 자원

이 검사는 결정적이므로 **엔진에 제약 검사기를 둔다(예정, `src/pok/engine/` — AD-3 위반 아님: 판단이 아니라 산수)**. 제약 원장의 규격은 [BUILD_DESIGN.md](BUILD_DESIGN.md) §2-3.

**③ 목표 상태 목적 (D28)** — 목적함수는 평평한 가중 합산이 아니라 **임계값·캡 충족**으로 표현한다. v6의 설계 판단 대부분이 "더 많이"가 아니라 "경계 지키기"(로우라이프 35% 경계·색상 과반·포인트 8 예산)였다 — 평평한 가중은 이런 경계·배타 구조를 표현하지 못함이 실증됐다. 다차원 목적 프로파일(§10.2)은 유지하되, **임계 충족 → 다음 병목**의 사전식(lexicographic) 구조가 우선한다.

**④ 앵커 모드와 설계 문서 (D26·D30)** — (a) 추천·최적화 모드(§10.1)의 앵커를 물리화한다: 검증된 외부 앵커 빌드를 `parse_pob`로 수집해 `artifacts/anchors/`에 계보 manifest와 함께 보관(RC5 대응 — 카피캣이 아니라 앵커+델타). 그리고 **design.md가 1급 산출물**이다 — PoB 코드는 상태의 스냅샷, 설계 문서는 판단·제약·가설의 기록(규격: [BUILD_DESIGN.md](BUILD_DESIGN.md)).

**⑤ 가설 관리 → P5 (D29)** — 설계 판단에도 KB 검증 라벨의 설계 버전인 **확정/잠정/미검증 3구분**을 적용한다. 미검증 상호작용은 계산에 선반영 금지(반프록시 AD-8의 설계 버전). **미검증 가설 목록 → 인게임 실험 → 결과 반영** 사이클이 P5 피드백 루프의 입력 형식이 된다.
...
> **§10.0과의 관계**: 이 파이프라인은 설계 루프의 실행 뷰이며, 특히 ⑧~⑫는 루프 5~6단계(확정·매칭)와 7단계(반복)에 대응한다. 내용은 그대로 유지된다 (D25).
...
> **§10.0과의 관계**: 트리 최적화는 빌드 생성의 중심이 아니라 **설계 루프 5~6단계 안에서 동작하는 도구 상자**다(D25). 설계 문서의 제약 원장(포인트 예산 등)을 전제 조건으로 받아, 그 안에서 연결·효율을 다룬다. 아래 내용은 그대로 유지된다.
...
§2 표의 D1~D24 전부 확정. **D25~D30(설계 루프 계층)은 사용자 협의(2026-07-31)된 방향의 초안 — 문서 확정은 사용자 검토 후.** 추가로: 엔진=결정적/지능=외부, KB 불변·학습 분리, 증거 위계·검증 라벨, 반프록시 원칙, 거래소 세션 쿠키 방식, 크로스플랫폼(Win+mac), 가격추산=요청 시, 버전관리=git만(스냅샷 폐기), 프로토타입=참고용만, 생성 전략(2모드·재조합)·트리 최적화(에이전트 결정+PoB 측정, 베이스라인→지시) 확정, "구조 먼저" 철칙.
...
> 다음 작업: **P0 부트스트랩 착수** ([ROADMAP.md](ROADMAP.md) §1).
