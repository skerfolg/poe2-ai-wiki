...
> **문서 상태**: v0.7 초안 (2026-07-31) — 빌드 생성의 중심을 "트리 최적화"에서 **"설계 루프(제약 충족 + 가설 관리)"**로 재정의 (§10.0, D25~D30, 사용자 협의 2026-07-31 — 사용자 검토 대기)
...
빌드 생성의 중심을 재정의한다: 중심은 "트리 최적화"가 아니라 **설계 루프 = 제약 충족 + 가설 관리**다. §10.1~10.3의 기존 결정(파이프라인·트리 최적화·RC1~5)은 전부 유지되며, 그 **상위에** 이 계층이 놓인다. 근거: 사용자의 실제 설계 실천(v6 문서, `artifacts/builds/20260731-ember-fusillade-설계v6/`)에서 검증된 작업 방식의 일반화.
