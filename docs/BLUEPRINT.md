...
> **문서 상태**: v0.6 (2026-07-29) — 프로젝트 구조·KB 데이터 모델·ingest·로드맵 확정 (상세는 분리 문서)
> **목적**: 코드 작성 전, 확정한 방향·아키텍처·계획을 **빠짐없이 기록**하고 남은 결정을 명시한다. 세부 구현이 아니라 **방향·러프 플랜** 수준. **이 문서가 프로젝트의 단일 진실 소스(source of truth)다.**
> **이력**: v0.1(도구형) → v0.2(엔진+MCP) → v0.3(프로토타입 회수) → v0.4(프로토타입=참고용, 스냅샷 폐기) → v0.5(생성 전략·트리 최적화 확정) → **v0.6(구조·KB모델·ingest·로드맵 확정, 패키지명 `pok`)**
> **상세 문서**: [PROJECT_STRUCTURE.md](PROJECT_STRUCTURE.md)(물리 구조) · [KB_DATA_MODEL.md](KB_DATA_MODEL.md)(KD-1~4) · [KB_INGEST.md](KB_INGEST.md)(KI-1~7) · [ROADMAP.md](ROADMAP.md)(P0~P6)
