...
| B-6 | **KB 쓰기 단일 경로** ⛔ *구조 변경 — 사용자 합의 필요* | **통합 브랜치 병합 완료 후 + 구조 변경 합의** | 문제: KB_DATA_MODEL §9가 `store.py`를 **로드·검증**으로만 정의하는데 **쓰기 계약이 없어**, ingest 모듈 17개가 각자 `knowledge/`에 쓰고 KD-1 배치 규칙(개별 JSON/NDJSON 샤드)을 매번 재추론한다 → 한 곳만 빠뜨려도 샤드 전체가 파괴된다(2026-08-02 실측 830건 손실). 계획: ① `store`에 쓰기 진입점 신설, KD-1 배치 판단을 **한 곳에 캡슐화** ② ingest·후처리 모듈(merge·tree_merge·mods·gem_costs·gem_colors 등)을 그 경로로 이관 ③ KB_DATA_MODEL §9에 쓰기 계약 명문화 | `knowledge/` 직접 쓰기가 0건(전부 store 경유), 파괴 회귀 테스트가 **단일 지점**에서 커버, §9에 계약 기록 |

> **병합 직후 처리 예정** (B-6과 별개, 구조 변경 아님 — 지시 대기): ① **모델 층** — `Record`가 샤드 소속을 표현하도록(`in_shard`, 통합 브랜치 `claude/elastic-merkle-504365` 접근 채택). 확장자 추론은 같은 판단의 반복이라 AD-7("구조적 결함은 구조에서")에 어긋난다. ② **삭제 정책** — merge의 "미포함 레코드 무조건 보존"(이 브랜치 임시 수정)을 걷어내고 **삭제 후보 리포트 + 사용자 판정**으로 교체. 근거: BLUEPRINT §7상 merge의 정상 사용은 패치 단위 전체 재수집이라 미포함 = 게임에서 제거되었을 수 있고, 사용자 확립 원칙("데이터를 지우기 전에 후보를 전량 나열해 게임 지식으로 판단")이 이미 답을 준다.
