...
> **문서 상태**: v0.3 (2026-08-02) — **P4.5 Exit 달성 기록** + §2-1 백로그 신설(잔여 항목의 착수 조건·계획·완료 기준). v0.2 (2026-07-31) P4.5 신설분, v0.1 (2026-07-29) 확정분 유지. BLUEPRINT §15-2 "재구축 로드맵/MVP"의 상세.
...
> **진행 상태**: ✅ **P4.5 Exit 달성 (2026-08-02, 사용자 승인)** — 빌드 설계 루프 완성. 블라인드 재현 4회로 검증(규율 도입 후 3·4차가 실제 인게임 좌초 원인 2건을 모델 등급 무관하게 무개입 검출). 잔여 항목은 §2-1 백로그. **다음 = P5 학습 루프** → ✅ P0 완료 → ✅ **P1a 완료** (2026-07-29, 시드 35·vocab v1·스키마·store 4층 검증·index self-healing, pytest 13 ✓, CI Win+mac green) → P1b 실질 완료(①~⑤ 가동) → **P3 Exit 달성 = ★MVP (2026-07-30)**: 사용자 자연어 요청(경계 감개 저주 스태킹 블러드 메이지) → KB 조회 → 후보 3안 → 선택안 3티어 조립(적법성 RC4 전 항목 통과) → PoB 실측 기록 (`artifacts/builds/20260730-liminal-curse-bolts-{저,중,고}/`, 저주 0→5개 DPS 2,584→16,858 델타 실측) → 공유 코드 전달 → **사용자 확인: '빌드 자체는 오류가 없습니다'**. 잔여(주얼 어댑터·트리/아이템 최적화)는 P4 범위 ← → **P2 Exit 달성 = M1 (2026-07-30)**: 사용자가 새 세션에서 '생명력 스태킹 노드' 자연어 질의 → pok MCP 도구로 KB 기반 답변 실증 (품질 승인). 후속 개선: FTS body 색인·토큰 AND·어센던시 실명 부착 — ①젬 + ②트리 + ③유니크 완료 → **KB 16,495 레코드**. 완전성 기준 8중 확장(⑥⑦⑧) 가동 + 그로 드러난 결함 4건 교정 완료. ④ 베이스+모드 풀 **완결** — 판정(185 승격·682 원장)·Desecrated 249·ko 이름(모드 2,004+ko효과 3,616·베이스 1,642·훼손 249)·GAME_DATA 승격 3,616·증거 체인 manifest(§5) 성립. crafting-rules 판 규칙 **확정**(접사 한도·화폐 동작 14종·카오스 고정 기법 인사이트, 사용자 판정). 제작 화폐 259종 수록(⑥에서 앞당김).
...
| B-5 | **v8 수동 대조군 처리** — 루프 출력물 원칙(BUILD_DESIGN §3-0) 확립 전에 손으로 갱신한 문서 | P5 착수 시 | `artifacts/builds/…-설계v8-수동대조군`은 채점 대조용으로만 보존, 정본 계보에서 분리 유지. 실제 설계 진행은 루프 출력물(run3/run4 산출)에서 이어간다 | 정본 계보에 수동 편집본이 섞이지 않음 |
