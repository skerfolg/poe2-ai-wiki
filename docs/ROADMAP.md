...
> **성격**: 전부 **흐름 결함이 아니라 흐름이 찾아낸 지식·파이프라인 공백**이다(흐름 결함은 0건 — 블라인드 4회 검증). 각 항목은 착수 조건·방법·완료 기준을 함께 적는다. 순서는 우선순위.

| # | 항목 | 착수 조건 | 진행 계획 (방법) | 완료 기준 |
|---|---|---|---|---|
| B-1 | **갑옷 정신력(Spirit) 접사 KB 갭** — 플랫/%/혼합/점유 효율 4종이 `check_item_legality` UNKNOWN. 완성 아이템셋 조립 자체가 막힘 | 즉시 (오프라인, 인게임 불요) | ① `artifacts/ingest-raw`의 poe2db 모드 원시로 갑옷 계열 Spirit 접사 실존 여부 판정 ② **실존 → ingest 보강**(mods 파이프라인 재실행·검증 라벨 부여) / **부재 → v6 갑옷 계획이 비실재 접사에 기댔다는 설계 결함**으로 판정해 사용자 보고 ③ 어느 쪽이든 판정 근거를 KB 또는 백로그에 기록 | `check_item_legality`가 갑옷 Spirit 접사에 UNKNOWN 대신 명확한 판정(LEGAL/CONDITIONAL/ILLEGAL) 반환, 또는 "게임에 없음" 판정이 문서화 |
| B-2 | **보조 젬 색상(요구 속성) 필드 미수록** — 색상 장부가 설계 문서 전사에 의존, KB 기계 검증 불가 | 즉시 (오프라인) | PoB 젬 데이터의 `reqStr/reqDex/reqInt`로 색상 도출(힘=빨강·민첩=초록·지능=파랑) → Support 레코드 `data.color` 수록. 무색·복합 케이스는 라벨 분리 | `check_constraints(color_ledger)` 입력을 KB 조회로 구성 가능(전사 불요) |
| B-3 | **과대망상 등 `pob_computable:false` 유니크의 조립 경로** — 원문 텍스트 조립 불가로 PoB 실측에서 제외됨 | 즉시 (파이프라인) | 노터블 직접 할당 방식(트리 노드 추가)으로 대체 조립하는 경로를 `assemble`에 추가하고, 계보에 "대체 모델링" 표기 | 과대망상 포함 빌드가 PoB 실측에 들어가고, 대체 방식이 manifest에 기록됨 |
