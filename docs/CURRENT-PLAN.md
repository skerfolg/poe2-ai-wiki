...
flowchart TD
  ...
  FIXDELTA[S15 #109 결측 키를 0으로 취급 수정<br/>next]
...
| S15 | #109 수정 — 결측 키를 0.0으로 취급하는 가짜 델타 | next |
...
| Integration branch override — 작업 브랜치 `docs/108-measure-defects` | `feat/105-tool-gaps`(다른 세션) 위에서 분기했다 — 백로그 번호 #108~#111이 그 세션의 #105~#107에 이어져야 해서다. 문서만 바꾼다. 이 PR 한정 | 2026-08-22 |
