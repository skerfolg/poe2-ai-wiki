...
측정 층은 닫혔다. 전량 재측정에서 **전 축 0인 노드가 936 → 583**으로 줄었고, 판정 41종 중
26종이 값을 냈다. 남은 15종은 config 미점등 3 + PoB 모델 갭 12로 **원인이 다 규명된** 것들이다.

이제 막힌 곳은 **정본 진입 경로가 없다는 것**이다 — 판정 41건이 쌓였는데 M6 큐레이션
게이트가 없어 정본은 안 움직인다.
