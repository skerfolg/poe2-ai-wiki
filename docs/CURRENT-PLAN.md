...
| S24 | #113 반영 재집계 — S20 재승격이 **수정 전에** 돌아 정본이 속도 없는 축으로 집계돼 있다. 재측정 불요(PoB 미사용) | next |

## Canonical Next Step

The only next executable step is: **#113 반영 재집계 (S24).**

S20은 닫혔다(PR #104) — 전량 재측정에서 전 축 0인 노드가 **936 → 583**으로 줄었고, 판정
41종 중 26종이 값을 냈다. 남은 15종은 config 미점등 3 + PoB 모델 갭 12로 원인이 다 규명됐다.

⛔ **그런데 그 재승격이 #113 수정(S23·PR #102)보다 먼저 돌았다.** 지금 정본 `NodeValue`
2,665종은 `showAverage` 빌드에서 **속도 배수가 빠진 `CombinedDPS`** 로 집계된 값이다 —
공격 속도 계열이 「0을 준다」로 남아 있다. 판정 큐·제안 다이제스트가 그 축을 읽으므로
그 위에서 판정하면 판정도 같이 틀린다.

비용은 작다: `collect()`는 **PoB를 안 쓴다**(데몬은 임시 헬퍼 `backfill_baselines`에만
있다). 저장된 행에 `TotalDPS`가 이미 실려 있어 **재측정 없이 재집계만** 다시 돌리면 된다.

그다음이 **M6 큐레이션 게이트**다 — 판정 41건이 쌓였는데 정본 진입 경로가 없어 정본이
안 움직인다. 측정 층이 닫힌 지금, 막힌 곳은 거기다.
