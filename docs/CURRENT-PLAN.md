...
| ⛔ **#113을 S20 재집계 전에 판정한다** | 측정은 전 축이지만(#108) **승격은 `CombinedDPS`로 순위를 매긴다**(`counterfactual_aggregate.py:394`). 「평균 피해 표시」 스킬에서 그 축이 속도를 안 먹으면 편향이 **정본 `NodeValue`에 박힌다** — 정본 오염은 되돌리기 어렵다(BLUEPRINT 함정 ②). S20 측정 데이터 자체는 무사하므로 **재집계 순서만 조정하면 된다** | 2026-08-24 |
| #113의 첫 걸음은 **수정이 아니라 검증** | §3에 같은 플래그를 「결함 마커」로 읽었다가 **91/96이 거짓 양성**으로 뒤집힌 기록이 있다. 정본 플래그 보유 **99종** vs #113 실측 근거 **1종**(`Rolling Slam`) — 대상을 확정하기 전에 고치면 §3에 한 줄이 더 는다 | 2026-08-24 |
