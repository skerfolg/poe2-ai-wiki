...
def test_진단_유형_넷이_사례와_함께_남는다(text: str) -> None:
    """§2-9의 유형 구분이 **사례 없이 남지 않게** 한다 (2026-08-25 신설).

    이 절이 생긴 이유: 「뒤집혔다」를 한 덩어리로 세다가 성격이 다른 넷을 뭉갰다.
    ⛔ 특히 **(d) 과소평가는 오류가 아니다** — 접수가 맞았고 착수가 더 찾은 것이다.
    그걸 오류로 세면 **정확한 접수를 한 사람이 벌을 받는다.**

    분류만 남고 사례가 사라지면 다음 세션이 어느 유형인지 못 고른다 — 그래서 사례를 함께 잠근다.
    """
    section = _section(text, "## 2-9. 진단이 틀리는", "## 3. 검증으로 뒤집힌 보고")
    for label in ("(a) 원인 가설", "(b) 범위", "(c) 문제 부재", "(d) 과소평가"):
        assert label in section, f"유형 「{label}」이 사라졌다"
    assert re.search(r"#\d+", section), "유형을 고르는 근거가 될 **실측 사례**가 없다"
    assert "오류가 아니다" in section or "뒤집힘」이 아니다" in section, (
        "(d)가 오류가 아니라는 단서가 사라졌다 — 그게 이 절의 핵심이다"
    )


def test_뒤집힌_보고_절이_범위_규율을_들고_있다(text: str) -> None:
    """§3 표에 **범위 칸이 없다**는 사실과 그 대응이 남아야 한다 (2026-08-25 신설).

    `show average damage` 플래그가 세 번 판정됐는데, **뒤의 둘은 서로 다른 질문에 답한
    둘 다 맞는 판정**이다. 범위가 없어 모순처럼 보였다 — 진동이 아니라 범위 누락이다.
    """
    section = _section(text, "## 3. 검증으로 뒤집힌 보고", "## 4. ")
