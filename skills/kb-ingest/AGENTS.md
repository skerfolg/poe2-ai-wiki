...
2. fetch 실행: `python -m poe2_wiki.kb.ingest fetch --patch <ver> --source <src>` (멱등 — 중복 실행 무해).
...
5. parse→match→merge→validate 실행: `python -m poe2_wiki.kb.ingest process --patch <ver>`.
